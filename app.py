...
st.markdown("""
    ...
    .st-expanderHeader { font-size: 22px !important; color: #ffffff !important; font-weight: bold !important; }
...
st.markdown("<p style='text-align:center; color:white; font-size:18px;'>Project Management System © 2026</p>", unsafe_allow_html=True)

st.write("---")

# --- 4. Main Information (Always Visible) ---
rig_name = st.text_input("Rig Name / Number", placeholder="e.g. Rig 77")
location_name = st.text_input("Location", placeholder="e.g. Buqayq")
dist = st.number_input("Rig Move Distance (KM)", value=0)
d_price = st.number_input("Diesel Price", value=1.70)

st.write("---")

# --- SECTION 1: RIG MOVE (البلس الأول) ---
with st.expander("➕ SECTION 1: RIG MOVE (Fleet & Support)"):
    ...
    with col1:
        ...
        st.write("---")
        n_flat_move = st.number_input("Flatbed Quantity", value=0, key="fm_q")
        p_flat_move = st.number_input("Flatbed Rate", value=0, key="fm_p")
    with col2:
        st.write("### Technical & Fuel")
...
# --- SECTION 2: OLD LOCATION (البلس الثاني) ---
with st.expander("➕ SECTION 2: OLD LOCATION"):
    ...
    with col3:
        ...
        p_crane_old = st.number_input("Crane Day Rate", value=0, key="co_p")
    with col4:
        st.write("### Personnel")
...
# --- SECTION 3: NEW LOCATION (البلس الثالث) ---
with st.expander("➕ SECTION 3: NEW LOCATION"):
    col5, col6 = st.columns(2)
    with col5:
        st.write("### Cranes")
        n_crane_2_new = st.number_input("2 Cranes (New) Qty", value=0, key="c2n_q")
        d_crane_2_new = st.number_input("2 Cranes Days", value=0, key="c2n_d")
        p_crane_2_new = st.number_input("2 Cranes Day Rate", value=0, key="c2n_p")
        st.write("---")
        n_crane_1_new = st.number_input("One Crane (New) Qty", value=0, key="c1n_q")
        d_crane_1_new = st.number_input("One Crane Days", value=0, key="c1n_d")
        p_crane_1_new = st.number_input("One Crane Day Rate", value=0, key="c1n_p")
    with col6:
        st.write("### Safety & TP")
...
if st.button("🚀 CALCULATE & EXCEL"):
    ...
    total_cost = fuel_cost + (n_lowbed*p_lowbed) + (n_flat_move*p_flat_move) + (n_workshop*d_workshop*p_workshop) + (n_tanker*d_tanker*p_tanker) + \
                 (n_crane_old*d_crane_old*p_crane_old) + (n_rigger_old*d_rigger_old*p_rigger_old) + \
                 (n_crane_2_new*d_crane_2_new*p_crane_2_new) + (n_crane_1_new*d_crane_1_new*p_crane_1_new) + \
                 (n_safety_new*d_safety_new*p_safety_new) + (n_tp_new*d_tp_new*p_tp_new)

    st.markdown(f"""
        <div class="neon-card">
            <h2 style="color:#00ffcc; margin:0;">TOTAL BUDGET: {total_cost:,.2f} SAR</h2>
            <p style="color:white; margin:0;">Diesel: {fuel_l:,.2f} L</p>
        </div>
    """, unsafe_allow_html=True)

    # --- Excel Export ---
    excel_data = {
        "Category": ["Rig Move", "Rig Move", "Rig Move", "Rig Move", "Old Loc", "Old Loc", "New Loc", "New Loc", "New Loc", "New Loc", "Fuel"],
        "Item": ["Lowbed", "Flatbed", "Workshop", "Tanker", "One Crane", "One Rigger", "2 Cranes", "One Crane", "Safety Man", "Truck Pusher", "Diesel"],
        "Total (SAR)": [(n_lowbed*p_lowbed), (n_flat_move*p_flat_move), (n_workshop*d_workshop*p_workshop), (n_tanker*d_tanker*p_tanker), (n_crane_old*d_crane_old*p_crane_old), (n_rigger_old*d_rigger_old*p_rigger_old), (n_crane_2_new*d_crane_2_new*p_crane_2_new), (n_crane_1_new*d_crane_1_new*p_crane_1_new), (n_safety_new*d_safety_new*p_safety_new), (n_tp_new*d_tp_new*p_tp_new), fuel_cost]
    }
